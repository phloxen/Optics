...
# 光波绘制
def draw_arc(painter, x_1, y_1, x_2, y_2, half_theta, r_reduce):
    # 获取当前画笔
    pen = painter.pen()
    painter.setPen(pen)
    x = x_2 - x_1
    y = y_2 - y_1
    # 计算 r
    r = np.sqrt(x ** 2 + y ** 2) - r_reduce
    # 计算 theta，以弧度为单位
    theta = np.arctan2(y, x)
    # 将 theta 转换为角度（如果需要）
    theta_degrees = - np.degrees(theta)
    start_angle = (theta_degrees - half_theta) * 16
    span_angle = half_theta * 2 * 16
...
class MyWindow(QWidget, Ui_Form):
    ...
    def bind(self):
        # 微光增强
        self.pushButton_glimmer.clicked.connect(self.GlimmerBrace)
        # 光波模式
    ...
    def GlimmerBrace(self):
        if self.isGlimmer:
            self.isGlimmer = False
            self.pushButton_glimmer.setText("标准光强")
            self.plant()
        else:
            self.isGlimmer = True  # 开启微光增强
            self.pushButton_glimmer.setText("微光增强")
            self.plant()
    ...
    def WaveChange(self):
        if self.radioButton.isChecked():
            ...
            self.timer.timeout.connect(self.WaveMove)
        ...
        if self.line_drawn:
            ...
            if self.isWave:  # 光波模式
                ...
                for y in [130, 140, 150, 160, 170, 180]:
                    draw_arc(qp, 40 + 50, y + 30, 80 + 50, y + 30, 30, self.r_reduce)
                # 绘制分光
                n = self.contants_2
                max_length = 96 / (n - 1)
                ...
                for y in [y_0[0], y_0[-1]]:
                    draw_arc(qp, 83 + 50, 155 + 30, x_0, y, 45, self.r_reduce)
                # 绘制干涉
                if self.delta_y == 0:
                    QMessageBox.information(self, '警告', '超出阈值', QMessageBox.StandardButton.Ok)
                    app.quit()
                for y in y_0:
                    draw_arc(qp, x_0, y, 333 + 50, 155 + 30, 40, self.r_reduce)
            else:  # 光线模式（默认）
                # 绘制入射光线
                for y in [130, 140, 150, 160, 170, 180]:
            ...
            for y in range(-125, 125):
                # 计算渐变值
                x = np.pi * y / self.delta_y
                n = self.contants_2
                if x == 0:
                    value = 255
                else:
                    if self.isGlimmer:
                        if (x % np.pi) <= (np.pi / n) or (x % np.pi) >= (np.pi - np.pi / n):
                            value = int((((np.sin(n * x)) ** 2) / ((n ** 2) * (np.sin(x)) ** 2)) * 255)
                        else:
                            value = int((((np.sin((3 * np.pi) / (2 * n))) ** 2) * ((np.sin(n * x)) ** 2) / (
                                    (np.sin(x)) ** 2)) * 255)
                            if value > 255:
                                value = 255
                    else:
